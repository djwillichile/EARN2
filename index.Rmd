...
Para llevar a cabo este procedimiento se deben seguir una serie de pasos consecutivos que permitan su correcto desarrollo. El primer paso consiste en consolidar una matriz o marco de datos de los objetos a clasificar con una cantidad y calidad adecuada de variables.Posterior a esto, es necesario construir una matriz de distancia o medidas de similitud con la cual comparar los disitntos registros de la base de datos para luego elegir un método adecuado de conglomeración y finalmente, validar el proceso de conglomerado

## Construcción de marco de datos
Cómo se comentó anteriormente la construcción del marco de datos es promordial para la realización del análisis de conglometados. Se requiere una base de datos que cuente con una estructura biend efinida, en la que las dstintas variables están dispuestas en columnas y los registros en filas, así mismo, para que el análisis se realice de una manera mucho más eficiente, es que se aconseja que la base de datos no posea más de 8 variables.

https://djwillichile.github.io/EARN2/DATA/PPM_stat.csv

### Importación de datos puntuales
Existe un gran número de formatos en que se puede importar datos, un ejemplo de estos es el ya conocido **csv** (*Comma Separated Values*), o el versátil **xlsx** de Excel, a continuación se importarón datos desde los formatos csv. Para estos ejemplos utilizaremos los materiales encontrados en el repositorio <https://github.com/djwillichile/EARN2/tree/main/DATA>, también pueden acceder a todo el material utilizado para esta actividad mediante el siguiente **[enlace](https://djwillichile.github.io/EARN2/DATA/PPM_stat.csv)**.
...
Prec <- read.table(file = fileStrig,sep = ",",dec = ".",header = T)
head(Prec)

# Leemos el archivo mediante la función "read.delim"
Prec <- read.delim(file = fileStrig,sep = ",",dec = ".",header = T)
head(Prec)
```
